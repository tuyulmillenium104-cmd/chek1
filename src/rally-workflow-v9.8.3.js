 ...
 * 
 * ═══════════════════════════════════════════════════════════════════════════
 * 🏛️ 3 AI JUDGES ARCHITECTURE (NEW IN v9.8.3)
 * ═══════════════════════════════════════════════════════════════════════════
 * 
 * ┌─────────────────────────────────────────────────────────────────────────┐
 * │  GATE 1 (JUDGE 1): CAMPAIGN REQUIREMENTS (20 pts, threshold 14)        │
 * │  ─────────────────────────────────────────────────────────────────────  │
 * │  Programmatic (Binary):                                                 │
 * │    • URL Present: 2 pts                                                 │
 * │    • Banned Words: 2 pts                                                │
 * │  AI-Based (Contextual):                                                 │
 * │    • Description: 4 pts • Rules: 4 pts • Style: 3 pts                   │
 * │    • Knowledge Base: 3 pts • Additional Info: 2 pts                     │
 * └─────────────────────────────────────────────────────────────────────────┘
 *                              │ ▼ FAIL FAST
 * ┌─────────────────────────────────────────────────────────────────────────┐
 * │  JUDGE 2: FACT-CHECK WITH [SRC: url] TAGS (5 pts, threshold 4)          │
 * │  ─────────────────────────────────────────────────────────────────────  │
 * │  • Every claim needs [SRC: url] tag • AI verifies each claim            │
 * └─────────────────────────────────────────────────────────────────────────┘
 *                              │ ▼ FAIL FAST
 * ┌─────────────────────────────────────────────────────────────────────────┐
 * │  JUDGE 3: QUALITY ASSESSMENT (80 pts, threshold 70)                     │
 * │  ─────────────────────────────────────────────────────────────────────  │
 * │  • Originality: 20 pts • Engagement: 20 pts • Clarity: 15 pts           │
 * │  • Emotional: 10 pts • X-Factor: 15 pts                                 │
 * └─────────────────────────────────────────────────────────────────────────┘
 *                              │ ▼
 *                    ┌─────────────────────┐
 *                    │ HIGHEST SCORE       │
 ...
 * 
 * ═══════════════════════════════════════════════════════════════════════════
 * 🆕 NEW IN v9.8.3 (Architecture Changes)
 * ═══════════════════════════════════════════════════════════════════════════
 * ✅ 3 AI Judges (reduced from 6) - Gate 1, Judge 2, Judge 3
 * ✅ FAIL FAST - Stop evaluation if any stage fails
 * ✅ [SRC: url] SOURCE TAGGING - Tag claims with sources, cleaned before output
 * ✅ HIGHEST SCORE SELECTION - Select best from all passing, not first passing
 * ✅ STAGE-BY-STAGE EVALUATION - Evaluate all contents per stage before next
 * ✅ MAX 5 REGENERATE CYCLES
 * ✅ QUALITY THRESHOLD 70/80 (87.5%)
 * 
 * ═══════════════════════════════════════════════════════════════════════════
 ...
 * ✅ Total Score System (105 pts max in v9.8.3)
 * ✅ SDK Only - No Fallbacks
 * ✅ Campaign Search by Name
 * ✅ Multi-Token Rate Limit Handler (11 tokens)
 * ✅ Python NLP Integration (VADER, TextBlob, spaCy, Sentence Transformers)
 * 
 * ═══════════════════════════════════════════════════════════════════════════
 * 📊 SCORING v9.8.3
 * ═══════════════════════════════════════════════════════════════════════════
 ...
 * | TOTAL          | 105    | 88        | 83.8%     |
 * 
 * BASED ON: v9.8.2 + Rally Ultimate Master Guide V3
 * 
 * Usage:
...
const CONFIG = {
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // V9.8.3: NEW 3 AI JUDGES THRESHOLDS
  // ═══════════════════════════════════════════════════════════════════════════
  thresholdsV983: {
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // V9.8.3 WORKFLOW SETTINGS
  // ═══════════════════════════════════════════════════════════════════════════
  workflowV983: {
    maxRegenerateCycles: 5,
    contentsPerCycle: 5,
    failFast: true,
    highestScoreSelection: true,
    stageByStageEvaluation: true,
    sourceTagging: true
  },
  
  // Legacy thresholds (kept for backward compatibility with v9.8.2 features)
...
// ============================================================================
// V9.8.3: 3 AI JUDGES ARCHITECTURE - FAIL FAST + HIGHEST SCORE SELECTION
// ============================================================================

/**
 * Clean [SRC: url] source tags from content before final output
 */
...
function checkUrlPresent(content, campaignUrl) {
  if (!campaignUrl) {
    return { passed: true, score: CONFIG.thresholdsV983.gate1.programmatic.urlPresent };
  }
  ...
  return {
    passed,
    score: passed ? CONFIG.thresholdsV983.gate1.programmatic.urlPresent : 0
  };
}
...
function checkBannedWords(content) {
  ...
  // Use the banned words from CONFIG if available, else use defaults
  const bannedWords = CONFIG.bannedWords?.patterns || [
  ...
  return {
    ...
    foundWords: found,
    score: found.length === 0 ? CONFIG.thresholdsV983.gate1.programmatic.bannedWords : 0
  };
}

/**
 * GATE 1 (JUDGE 1): Campaign Requirements
 * Programmatic: URL Present, Banned Words
 * AI-Based: Description, Rules, Style, Knowledge Base, Additional Info
 */
async function gate1JudgeV983(llm, content, campaignData) {
  console.log('\n   🔍 GATE 1: Campaign Requirements Judge');
  
  const thresholds = CONFIG.thresholdsV983.gate1;
  const results = {
    programmatic: {},
    aiBased: {},
    totalScore: 0,
  ...
  console.log(`      ${urlResult.passed ? '✅' : '❌'} URL Present: ${urlResult.score}/${thresholds.programmatic.urlPresent}`);
  if (!urlResult.passed) {
  ...
  console.log(`      ${bannedResult.passed ? '✅' : '❌'} Banned Words: ${bannedResult.score}/${thresholds.programmatic.bannedWords}`);
  ...
  const aiPrompt = `You are Gate 1 AI Judge for Rally.fun. Evaluate this content against campaign requirements.
...
Respond ONLY in JSON format:
{
  ...
  try {
    const aiResponse = await llm.chat([{ role: 'user', content: aiPrompt }], { temperature: 0.2 });
    ...
    if (jsonMatch) {
      ...
      console.log(`      📝 Description: ${results.aiBased.description?.score || 0}/${thresholds.aiBased.description}`);
      console.log(`      📋 Rules: ${results.aiBased.rules?.score || 0}/${thresholds.aiBased.rules}`);
      console.log(`      🎨 Style: ${results.aiBased.style?.score || 0}/${thresholds.aiBased.style}`);
      console.log(`      📚 KB: ${results.aiBased.knowledgeBase?.score || 0}/${thresholds.aiBased.knowledgeBase}`);
      console.log(`      ➕ Additional: ${results.aiBased.additionalInfo?.score || 0}/${thresholds.aiBased.additionalInfo}`);
  ...
  // Determine if passed
  results.passed = results.totalScore >= thresholds.pass;
  
  console.log(`\n   📊 GATE 1 TOTAL: ${results.totalScore}/${thresholds.max} (${results.passed ? '✅ PASS' : '❌ FAIL'})`);
  
  return results;
}

/**
 * JUDGE 2: Fact-Check with [SRC: url] Source Tagging
 */
async function judge2FactCheckV983(llm, content, researchData) {
  console.log('\n   🔍 JUDGE 2: Fact-Check with Source Tags');
  
  const thresholds = CONFIG.thresholdsV983.judge2;
  const results = {
    claims: [],
    verifiedCount: 0,
    totalScore: 0,
...
List up to ${thresholds.maxClaims} specific, verifiable claims.
...
Only include OBJECTIVE claims that can be verified, not opinions or subjective statements.`;

  try {
    const claimResponse = await llm.chat([{ role: 'user', content: claimPrompt }], { temperature: 0.2 });
    const claimMatch = claimResponse.match(/\{[\s\S]*\}/);
    
    if (!claimMatch) {
      console.log('      ⚠️ No claims extracted');
      results.passed = true;
      results.totalScore = thresholds.max;
      return results;
    }
    ...
    for (let i = 0; i < Math.min(claims.length, thresholds.maxClaims); i++) {
      ...
      let searchResults = [];
      try {
        searchResults = await llm.webSearch(claim.claim);
      } catch (e) {
...
Respond ONLY in JSON:
{
  ...
  "explanation": "..."
      ...
      const verifyResponse = await llm.chat([{ role: 'user', content: verifyPrompt }], { temperature: 0.2 });
      ...
      await delay(CONFIG.delays.betweenQuickJudge || 2000);
    }
    
    // Calculate score
    results.totalScore = results.verifiedCount;
    results.passed = results.verifiedCount >= thresholds.pass;
    
    console.log(`\n   📊 JUDGE 2 TOTAL: ${results.verifiedCount}/${thresholds.max} verified (${results.passed ? '✅ PASS' : '❌ FAIL'})`);
 ...
 */
async function judge3QualityV983(llm, content, campaignData, competitorContents = []) {
  console.log('\n   🔍 JUDGE 3: Quality Assessment');
  
  const thresholds = CONFIG.thresholdsV983.judge3;
  const results = {
    components: {},
    totalScore: 0,
    passed: false,
  ...
  // Use G4 and X-Factor detection from v9.8.2
  const g4Result = detectG4Elements(content);
  const xFactorResult = detectXFactors(content);
  
  const qualityPrompt = `You are Judge 3: Quality Assessment for Rally.fun. Score this content.

CAMPAIGN: ${campaignData.title || 'N/A'}
...
G4 ORIGINALITY ANALYSIS (already detected):
- Casual Hook: ${g4Result.bonuses?.casualHookOpening ? '✅' : '❌'}
- Parenthetical Aside: ${g4Result.bonuses?.parentheticalAside ? '✅' : '❌'}
- Contractions: ${g4Result.bonuses?.contractions?.count || 0} found
- Personal Angle: ${g4Result.bonuses?.personalAngle ? '✅' : '❌'}
- Conversational Ending: ${g4Result.bonuses?.conversationalEnding ? '✅' : '❌'}

X-FACTOR ANALYSIS (already detected):
- Specific Numbers: ${xFactorResult.detected?.some(x => x.type === 'Specific Numbers') ? '✅' : '❌'}
- Time Specificity: ${xFactorResult.detected?.some(x => x.type === 'Time Specificity') ? '✅' : '❌'}
- Embarrassing Honesty: ${xFactorResult.detected?.some(x => x.type === 'Embarrassing Honesty') ? '✅' : '❌'}
...
Respond ONLY in JSON:
{
  ...
  try {
    const response = await llm.chat([{ role: 'user', content: qualityPrompt }], { temperature: 0.2 });
    ...
    if (jsonMatch) {
      ...
      console.log(`      ✨ Originality: ${results.components.originality?.score || 0}/${thresholds.components.originality}`);
      console.log(`      🎯 Engagement: ${results.components.engagement?.score || 0}/${thresholds.components.engagement}`);
      console.log(`      📝 Clarity: ${results.components.clarity?.score || 0}/${thresholds.components.clarity}`);
      console.log(`      💫 Emotional: ${results.components.emotional?.score || 0}/${thresholds.components.emotional}`);
      console.log(`      ⭐ X-Factor: ${results.components.xFactor?.score || 0}/${thresholds.components.xFactor}`);
  ...
  console.log(`\n   📊 JUDGE 3 TOTAL: ${results.totalScore}/${thresholds.max} (${results.passed ? '✅ PASS' : '❌ FAIL'})`);
  
  return results;
}

/**
 * Evaluate content with 3 AI Judges + Fail Fast
 */
async function evaluateWith3Judges(llm, content, campaignData, researchData, competitorContents) {
  console.log('\n════════════════════════════════════════════════════════════');
  console.log('⚖️  3 AI JUDGES EVALUATION (v9.8.3)');
  console.log('════════════════════════════════════════════════════════════');
  ...
  evaluation.gate1 = await gate1JudgeV983(llm, content, campaignData);
  ...
  await delay(CONFIG.delays.betweenJudges || 3000);
  
  // JUDGE 2: Fact-Check
  evaluation.judge2 = await judge2FactCheckV983(llm, content, researchData);
  
  if (!evaluation.judge2.passed) {
    evaluation.failFastStage = 'JUDGE_2';
    evaluation.feedback.push(`Judge 2 failed: Only ${evaluation.judge2.verifiedCount}/${CONFIG.thresholdsV983.judge2.pass} claims verified`);
  ...
  // JUDGE 3: Quality Assessment
  evaluation.judge3 = await judge3QualityV983(llm, content, campaignData, competitorContents);
  
  if (!evaluation.judge3.passed) {
    evaluation.failFastStage = 'JUDGE_3';
    evaluation.feedback.push(`Judge 3 failed: Score ${evaluation.judge3.totalScore}/${CONFIG.thresholdsV983.judge3.pass} below threshold`);
  ...
  console.log(`\n   ✅ PASSED ALL 3 JUDGES! Total Score: ${evaluation.totalScore}/${CONFIG.thresholdsV983.total.max}`);
  
  return evaluation;
}

/**
 * Stage-by-Stage Evaluation - Evaluate all contents per stage before moving to next
 */
async function stageByStageEvaluationV983(llm, contents, campaignData, researchData, competitorContents) {
  ...
  for (let i = 0; i < contents.length; i++) {
    ...
    const gate1Result = await gate1JudgeV983(llm, contents[i], campaignData);
  ...
  for (const item of passedGate1) {
    ...
    const judge2Result = await judge2FactCheckV983(llm, item.content, researchData);
  ...
  for (const item of passedJudge2) {
    console.log(`\n   Evaluating Content ${item.index + 1}...`);
    const judge3Result = await judge3QualityV983(llm, item.content, campaignData, competitorContents);
...
/**
 * Select content with HIGHEST score (not first passing)
 */
function selectHighestScoreV983(stageResults) {
