...
# Versi 3.0 | 9 April 2026
# v3.0 changes: Real-Time Intelligence (web-search, agent-browser, web-reader)
...
6. Lanjutkan dari mana terakhir berhenti
...
7. `/home/z/my-project/download/rally_system/rally_qna.json`
...
10. `/home/z/my-project/download/rally_skill_v11.7_SKILL.md`
...
```
Rally Content Generator v2.0

Pilih:
1. Mulai campaign baru
2. Stop campaign aktif
3. Cek status
4. Ambil konten siap copy-paste
5. Update skor Rally asli
6. Ambil Q&A (jawaban reply)
...
### Jika user pilih 2 (Stop):
- Set pipeline_state: "stopped" di rally_master.json
- Cron job akan skip generate tapi tetap update pattern cache
- Tampilkan: "Berhenti. Ketik /rally lagi kapan saja untuk mulai baru."

### Jika user pilih 3 (Status):
- Baca rally_master.json
- Tampilkan: campaign, mission, skor terbaik, total cycle, status (stopped/working/ready), error terakhir
...
### Jika user pilih 4 (Ambil konten):
- Baca rally_best_content.txt
- Jika sudah 18/18: tampilkan konten lengkap siap copy-paste
- Jika belum 18/18: tampilkan status + "Belum dapat 18/18. Skor terbaik: X/18."

### Jika user pilih 5 (Update skor):
...
- `/rally stop` → sama seperti pilih 2
- `/rally status` → sama seperti pilih 3
- `/rally score [X]/18` → sama seperti pilih 5
- `/rally qna` → sama seperti pilih 6
...
## PIPELINE YANG HARUS DIJALANKAN (17 STEP - v3.0)

### PRE-PIPELINE: Load Knowledge (v2.0)
...
- Identifikasi pelajaran yang relevan:
  ...
  * Calibration: apakah internal judge bias?
...
### Step 4: Generate 3 Variations (Step 6 dari skill v11.7)
- Variation A: Free Write (personal voice, no template)
- Variation B: Targeted (specific angle dari gap analysis + knowledge_vault)
- Variation C: Free Write (different personal angle)
- WAJIB: personal anecdote, specific detail, genuine voice
...
5. rally_qna.json (kalau generate Q&A - WAJIB setelah 18/18)
...
## CRON JOB BEHAVIOR (v3.0)

### MODE 1: BUILD (setelah user pilih "Mulai")
- BACA knowledge_vault.json + learning_log.jsonl SEBELUM generate (PRE-PIPELINE)
- Step 0: Real-Time Intelligence → BARU di v3.0
...
- Setiap cycle: load knowledge → intel check → fetch data → analyze → generate → judge
- Kalau belum 18/18: feedback loop (pakai pelajaran dari learning_log) → generate lagi
- Kalau SUDAH 18/18:
  1. Simpan ke rally_best_content.txt
  2. JALANKAN Q&A Generator (Step 12) → simpan ke rally_qna.json
...
## CONTINUOUS LEARNING SYSTEM (v2.0)
...
### Self-Improvement per Cycle (v2.0)
...
4. Terapkan rules tsb ke content generation
...
1. Baca /home/z/my-project/download/rally_system/RALLY_README.txt
2. Baca /home/z/my-project/download/rally_system/rally_master.json
3. Baca /home/z/my-project/download/rally_system/rally_worklog.md
4. Baca /home/z/my-project/download/rally_system/rally_skill_v11.7_SKILL.md
5. Cek pipeline_state DAN campaign deadline:
   ...
   - "content_ready" → jalankan MODE 2 MONITOR (lihat README v3.0 untuk detail), SELESAI
   - "needs_first_run" atau "working" → jalankan MODE 1 BUILD (lihat README v3.0 untuk detail), SELESAI

=== V3.0: REAL-TIME INTELLIGENCE (WAJIB) ===
6. Step 0 - Intel Gathering (JALANKAN SEBELUM generate/monitor):
...
=== PIPELINE LANJUTAN ===
7. Jalankan pipeline sesuai mode (BUILD atau MONITOR) dari README v3.0
8. Setiap selesai → update SEMUA file:
   ...
   - rally_qna.json (kalau generate Q&A - WAJIB setelah 18/18)
   - rally_all_variations.json (kalau buat variasi baru)
...
9. JANGAN fabrikasi data. JANGAN pura-pura berhasil. JANGAN skip step.
10. Jika error → catat ke learning log + worklog → retry SEKALI → jika tetap error → skip cycle ini.
11. BELAJAR TERUS: baca knowledge vault + learning log + intel cache sebelum setiap generate. Terapkan pelajaran.
