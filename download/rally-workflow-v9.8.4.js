/**
 * ═══════════════════════════════════════════════════════════════════════════
 * RALLY WORKFLOW V9.8.4 — ANTI-AI HUMAN-LIKE PIPELINE (v9.8.3 + v6 merged)
 * ═══════════════════════════════════════════════════════════════════════════
 *
 * 🆕 v9.8.4 CHANGES (from v9.8.3 + v6 anti-AI merge):
 * ✅ KB Data Extraction — facts whitelist + claims blacklist (anti-fabrication)
 * ✅ 5 Non-Linear Narrative Structures (Doubt-First, Mid-Thought, Tangent-Entry, Fragment-Storm, Asymmetric)
 * ✅ Human Artifacts Injection (tangential digression, self-correction, genuine uncertainty, run-on sentences)
 * ✅ Mandatory Counter-Argument / Doubt in every variant
 * ✅ Forbidden AI Patterns (expanded 40+ red flag phrases)
 * ✅ External Forensic Audit — independent AI probability check (reject >50%)
 * ✅ Anti-Over-Optimization Check — programmatic guardrails
 * ✅ Anti-Fabrication Rules — ONLY use KB data, blacklist fake claims
 *
 * ✅ ALL v9.8.3 FEATURES KEPT INTACT:
 * ✅ Rally API Integration (fetch campaigns, leaderboard, submit)
 * ✅ 6 Detailed Judge Prompts (getJudge1-6SystemPrompt)
 * ✅ Learning System (failure analysis, adaptive prompting, deduplication)
 * ✅ Deep Competitor Analysis (TOP 20 leaderboard benchmarking)
 * ✅ Dual AI Mode (SDK + HTTP Direct via fetch)
 * ✅ Multi-Mode CLI (name/address/list/manual)
 * ✅ Rich Display (MindsetFramework, ControlMatrix, TokenPool)
 * ✅ Persona/Emotion/Audience selection + rotation
 * ✅ CP Prediction, Q&A Generation, Read-Aloud, Who-Wrote-This
 *
 * Usage:
 ...
 *   node rally-workflow-v9.8.4.js manual campaigns/rally-beta.json
 */
...
// AI Worker path — child process for isolated AI calls (K8s-safe)
...
// Gateway endpoints - BOTH ALIVE! Dual gateway = 2x rate limit capacity
// ip-10min counter is PER gateway+token (verified via response headers)
// 11 tokens x 2 gateways = 22 buckets x 10 req/10min = 220 req/10min
const GATEWAY = {
...
const CONFIG = {
  ...
  model: {
    name: 'glm-5',
    enableThinking: true,
    enableSearch: true,  // Child process handles long requests — K8s-safe
    temperature: {
      generation: 0.8,
      judging: 0.2,
      compliance: 0.1
...
class MultiProviderLLM {
  ...
  async blindJudge(systemPrompt, userPrompt, judgeId, options = {}) {
    ...
    ], { temperature: 0.2, maxTokens: 3000, model: 'glm-5', enableSearch: false }); // No search needed for blind judging
    
    try { console.log(`   ✅ Judge ${judgeId} success! (model: glm-5)`); } catch (e) { /* EPIPE safe */ }
  ...
  async contextAwareJudge(systemPrompt, userPrompt, judgeId) {
    ...
    const result = await callAI([
      { role: 'system', content: systemPrompt },
      { role: 'user', content: userPrompt }
    ], { temperature: 0.2, maxTokens: 3000, model: 'glm-5', enableSearch: false }); // No search needed for context-aware judging
    
    try { console.log(`   ✅ Context-Aware Judge ${judgeId} success! (model: glm-5)`); } catch (e) { /* EPIPE safe */ }
  ...
  async factCheckJudge(systemPrompt, userPrompt, judgeId) {
    ...
    // Fact-check with web search (child process handles the long request — K8s-safe)
    const result = await callAI([
      { role: 'system', content: systemPrompt },
      { role: 'user', content: userPrompt }
    ], { temperature: 0.2, maxTokens: 3000, model: 'glm-5', enableSearch: true });
    
    try { console.log(`   ✅ Fact-Check Judge ${judgeId} success! (model: glm-5 + web search)`); } catch (e) { /* EPIPE safe */ }

    return {
      content: result.content || '',
      thinking: result.thinking || null,
      webSearchUsed: true, // Web search via child process (K8s-safe)
      provider: result.provider,
      model: 'glm-5'
    };
  }
...
`;
    ...
    ], { temperature: 0.2, maxTokens: 3000, model: 'glm-5', enableSearch: false }); // NLP data sufficient

    console.log(`   ✅ Hybrid Judge ${judgeId} success! (model: glm-5)`);
...
Return JSON:
{
  ...
  const response = await Promise.race([
    llm.chat([
      { role: 'system', content: systemPrompt },
      { role: 'user', content: userPrompt }
    ], { temperature: 0.9, maxTokens: 2000, model: 'glm-5', enableSearch: false }),
    new Promise((_, reject) => setTimeout(() => reject(new Error('Perspective timeout (60s)')), 60000))
  ]);
...
` : '';
  ...
  // 🆕 v9.8.4: Extract KB data for anti-fabrication + build anti-AI section
  const kbData = extractKBData(campaignData.knowledgeBase || campaignData.knowledge_base);
  const antiAI = buildAntiAISection(Math.floor(Math.random() * 5), kbData, researchData);
...
Write content that makes readers STOP scrolling, FEEL something, and ENGAGE.`;
  ...
  const response = await llm.chat([
    { role: 'system', content: systemPrompt },
    { role: 'user', content: userPrompt }
  ], { temperature: 0.8, maxTokens: 4000, model: 'glm-5', enableSearch: true }); // Web search via child process
...
function validateCampaignRequirements(content, campaignData) {
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // 🚨 CHECK 9: RALLY BANNED PHRASES (FATAL - CC Gate Disqualification)
  // These phrases cause automatic DISQUALIFICATION by Rally's CC gate
  // ═══════════════════════════════════════════════════════════════════════════
  const rallyBanned = CONFIG.hardRequirements?.rallyBannedPhrases || [];
  if (rallyBanned.length > 0) {
    const foundBannedPhrases = [];
    ...
    if (foundBannedPhrases.length > 0) {
      validation.checks.rallyBannedPhrases = {
        ...
        passed: false,
        fatal: true,
        message: `🚨 FATAL: Rally banned phrases found → CC Gate DISQUALIFICATION`
      };
      validation.missingElements.push(`🚨 BANNED PHRASE: ${foundBannedPhrases.join(', ')}`);
      validation.prohibitedItemsFound.push(`🚨 RALLY BANNED: ${foundBannedPhrases.join(', ')}`);
      validation.passed = false;
    } else {
      validation.checks.rallyBannedPhrases = {
        found: [],
        passed: true,
        message: '✅ No Rally banned phrases detected'
      };
    }
  }
...
async function judgeContentFailFast(content, campaignData, competitorContents, contentIndex, cycleNumber, state = judgingState) {
  ...
  const checksToDisplay = [
    ...
    { key: 'rallyBannedPhrases', label: 'Rally Banned Phrases', icon: '🚨' }
  ];
...
Return JSON:
{
  ...
  try {
    ...
    ], { temperature: 0.2, maxTokens: 4000, model: 'glm-5', enableSearch: false }); // No search needed
...
Return JSON:
{
  ...
  try {
    ...
    ], { temperature: 0.3, maxTokens: 2000, model: 'glm-5', enableSearch: false }); // No search needed
...
async function runFirstPassWorkflow(campaignInput, missionNumber = null) {
  ...
  // Fetch competitor submissions (with timeout to avoid hanging forever)
  ...
  const submissions = await Promise.race([
    ...
    new Promise(resolve => setTimeout(() => { console.log('   ⏰ Leaderboard timed out (15s)'); resolve([]); }, 15000))
  ]);
  ...
  while (!judgingState.hasWinner() && cycleNumber < maxCycles) {
    ...
    // FIXED: SEQUENTIAL generation (not parallel) to avoid rate limit cascade
    console.log('\n📝 Generating contents SEQUENTIALLY (avoiding rate limits)...');
    const validContents = [];
    for (let i = 0; i < contentsPerCycle; i++) {
      if (judgingState.hasWinner()) break;
      console.log(`\n   📝 Generating Content ${i + 1}/${contentsPerCycle}...`);
      try {
        const genResult = await generateSingleContentForParallel(campaignData, competitorAnalysis, researchData, totalGenerated + i, comprehensionPlan, cycleLearningInsights);
        if (genResult && genResult.success && genResult.content) {
          validContents.push(genResult);
          totalGenerated++;
          console.log(`   ✅ Content ${i + 1} generated successfully (${genResult.content.length} chars)`);
        } else {
          totalFailed++;
          console.log(`   ❌ Content ${i + 1} generation failed`);
        }
      } catch (genErr) {
        totalFailed++;
        console.log(`   ❌ Content ${i + 1} error: ${genErr.message}`);
      }
      // Stagger delay between generations
    ...
    if (validContents.length === 0) {
      console.log('   ⚠️ No contents generated this cycle, retrying...');
      await delay(3000);
      continue;
    }
    
    console.log(`\n   ✅ Generated ${validContents.length}/${contentsPerCycle} contents`);
    
    // FIXED: SEQUENTIAL judging (not parallel) to avoid rate limit cascade
    console.log('\n⚖️ Judging contents SEQUENTIALLY...');
    const judgeResultsThisCycle = [];
    for (const genResult of validContents) {
      if (judgingState.hasWinner()) break;
      console.log(`\n   ⚖️ Judging Content ${genResult.index + 1}...`);
      try {
        const jResult = await judgeContentFailFast(genResult.content, campaignData, competitorContents, genResult.index, cycleNumber, judgingState);
        judgeResultsThisCycle.push(jResult);
        if (jResult.passed) {
          judgingState.setWinner(jResult);
          break;
        }
      } catch (judgeErr) {
        console.log(`   ⚠️ Judge error: ${judgeErr.message}`);
        judgeResultsThisCycle.push({ content: genResult.content, passed: false, failedAt: 'error' });
      }
      // Delay between judges
      if (!judgingState.hasWinner()) {
        console.log(`   ⏳ Waiting 5s before next judge...`);
        await delay(5000);
      }
    }
    
    allCycleJudgeResults.push(...judgeResultsThisCycle);
    
    // Count failures
    const failedThisCycle = judgeResultsThisCycle.filter(r => !r.passed && !r.skipped).length;
  ...
  // ═══════════════════════════════════════════════════════════════
  // 🆕 v9.8.4: EXTERNAL FORENSIC AUDIT + ANTI-OVER-OPTIMIZATION
  // ═════════════════════════════════════════════════════════════════════════════
  console.log('\n🔍 Running External Forensic Audit (AI probability check)...');
  const kbDataAudit = extractKBData(campaignData.knowledgeBase || campaignData.knowledge_base);
  const auditResult = await externalForensicAudit(winner.content, campaignData, kbDataAudit.factsWhitelist);
  console.log(`     AI Probability: ${auditResult.aiProbability}% | Human: ${auditResult.humanProbability || '?'}% | Uniqueness: ${auditResult.uniqueness || '?'}% | Rejected: ${auditResult.rejected}`);
  if (auditResult.rejected) {
    console.log(`     ⚠️ REJECTED — Reasons: ${(auditResult.reasons || []).slice(0, 3).join('; ')}`);
    console.log('     Content passed Rally judges but may be flagged by external audit.');
  }

  console.log('\n🛡️  Running Anti-Over-Optimization Check...');
  const antiOptResult = antiOptimizationCheck(winner.content);
  console.log(`     Score: ${antiOptResult.score}/${antiOptResult.maxScore} | Passed: ${antiOptResult.passed}`);
  if (antiOptResult.penalties.length > 0) {
    console.log(`     Penalties: ${antiOptResult.penalties.map(p => p.description).join('; ')}`);
  }

  // Save audit results
  try {
    fs.writeFileSync(`${CONFIG.outputDir}/audit-result-${Date.now()}.json`, JSON.stringify({ auditResult, antiOptResult, winnerContent: winner.content }, null, 2));
  } catch (e) { /* ignore */ }
  ...
  return {
    ...
    // 🆕 v9.8.4: Include audit results
    auditResult,
    antiOptimizationResult: antiOptResult
  };
}
...
Return JSON:
{
  "engagement_qa": [
    ...
    ], { maxTokens: 4000, temperature: 0.7, model: 'glm-5-flash', enableSearch: false }); // No search needed
...
async function runManualWorkflow(jsonFilePath) {
  ...
  // FETCH COMPETITOR from Rally API (proper endpoint from Elite Masterclass PDF)
  ...
  if (campaignData.intelligentContractAddress) {
    ...
    try {
      ...
      console.log(`   📊 Found ${submissions?.length || 0} submissions`);
      if (submissions && submissions.length > 0) {
        // Extract text from leaderboard data directly (no AI call needed)
        competitorContents = submissions.slice(0, 10).map(s => {
          const user = s.user || {};
          return `@${user.xUsername || 'unknown'} (Rank #${s.rank}, Score: ${(s.points / 1e18).toFixed(2)})`;
        });
        console.log(`   ✅ Extracted ${competitorContents.length} competitor profiles from leaderboard`);
        competitorAnalysis = { competitorContent: competitorContents, analysis: `Extracted from ${submissions.length} leaderboard entries` };
      } else {
        console.log('   ℹ️ No submissions found — proceeding without competitor data');
        competitorAnalysis = { competitorContent: [], analysis: 'No submissions' };
      }
    } catch (e) {
      console.log(`   ⚠️ Competitor fetch failed: ${e.message} — continuing without`);
  ...
  // FIXED: Add missing research data (was missing → ReferenceError crash)
  console.log('\n🔎 Running Multi-Query Deep Research...');
  let researchData;
  try {
    console.log('   [DEBUG] Creating MultiProviderLLM instance...');
    const llmResearch = new MultiProviderLLM(CONFIG);
    console.log('   [DEBUG] LLM instance created, calling multiQueryDeepResearch...');
    researchData = await multiQueryDeepResearch(llmResearch, campaignData.title, campaignData);
    console.log(`   ✅ Research complete: ${researchData ? 'OK' : 'NULL'}`);
  } catch (researchErr) {
    console.log(`   ⚠️ Research failed: ${researchErr.message}`);
    console.log(`   ⚠️ Stack: ${researchErr.stack?.substring(0, 200) || 'N/A'}`);
    console.log('   ⚠️ Continuing without research data...');
    researchData = null;
  }

  // FIXED: Add missing comprehension plan (was missing → undefined)
  let comprehensionPlan = null;
  try {
    const llmComp = new MultiProviderLLM(CONFIG);
    console.log('\n🧠 Running Campaign Comprehension Check...');
    comprehensionPlan = await campaignComprehensionCheck(llmComp, campaignData, competitorAnalysis, researchData, parseCampaignRequirements(campaignData));
    console.log(`   ✅ Comprehension: ${comprehensionPlan.understood ? 'PASS' : 'NEEDS ATTENTION'}`);
  } catch (compErr) {
    console.log(`   ⚠️ Comprehension failed: ${compErr.message} — continuing without`);
    comprehensionPlan = null;
  }

  console.log('\n📊 STARTING CONTENT GENERATION PHASE...');
  console.log(`   Competitor Analysis: ${competitorAnalysis ? 'OK' : 'NULL'}`);
  console.log(`   Research Data: ${researchData ? 'OK' : 'NULL'}`);
  console.log(`   Comprehension Plan: ${comprehensionPlan ? 'OK' : 'NULL'}`);
  ...
  while (!judgingState.hasWinner() && cycleNumber < maxCycles) {
    ...
    for (let i = 0; i < contentsPerCycle; i++) {
      ...
      console.log(`\n   📝 Generating Content ${i + 1}/${contentsPerCycle}...`);
      try {
        const genResult = await generateSingleContentForParallel(campaignData, competitorAnalysis, researchData, totalGenerated + i, comprehensionPlan, null);
        if (genResult && genResult.success && genResult.content) {
          validContents.push(genResult);
          totalGenerated++;
          console.log(`   ✅ Content ${i + 1} generated successfully (${genResult.content.length} chars)`);
    ...
    if (validContents.length === 0) { console.log('   ⚠️ No contents generated, retrying...'); await delay(3000); continue; }
    ...
    for (const genResult of validContents) {
      ...
      try {
        const jResult = await judgeContentFailFast(genResult.content, campaignData, competitorContents, genResult.index, cycleNumber, judgingState);
        judgeResultsThisCycle.push(jResult);
        if (jResult.passed) { judgingState.setWinner(jResult); break; }
      } catch (judgeErr) {
        console.log(`   ⚠️ Judge error: ${judgeErr.message}`);
        judgeResultsThisCycle.push({ content: genResult.content, passed: false, failedAt: 'error' });
      }
...
// ═══════════════════════════════════════════════════════════════════════════════
// 🆕 v9.8.4: ANTI-AI FEATURES (from v6 pipeline)
// ═══════════════════════════════════════════════════════════════════════════════

/**
 * KB Data Extraction — Extract facts whitelist + claims blacklist from knowledge base
 * Prevents content fabrication by only allowing KB-sourced data
 */
function extractKBData(knowledgeBase) {
  if (!knowledgeBase) return { factsWhitelist: [], claimsBlacklist: [], extractedNumbers: [], extractedUrls: [] };

  const kb = knowledgeBase;

  // Extract numbers
  const numberMatches = kb.match(/\b\d+(?:\.\d+)?%?\b/g) || [];
  const extractedNumbers = [...new Set(numberMatches)];

  // Extract URLs
  const urlMatches = kb.match(/https?:\/\/[^\s)\]"]+/g) || [];
  const extractedUrls = [...new Set(urlMatches)];

  // Build facts whitelist: factual sentences from KB
  const sentences = kb.split(/[.\n]+/).map(s => s.trim()).filter(s => s.length > 15 && s.length < 200);
  const rawFacts = sentences.filter(s =>
    /\b(is|are|was|were|has|have|can|will|uses|distributes|prioritizes|shifts)\b/i.test(s) || /\d/.test(s)
  );

  // Claims blacklist: patterns that indicate fabricated data
  const claimsBlacklist = [
    'Do NOT claim specific follower counts or user numbers not in the KB',
    'Do NOT claim specific TVL, market cap, or revenue figures not in the KB',
    'Do NOT claim specific percentages or growth rates not in the KB',
    'Do NOT claim partnerships not mentioned in the KB',
    'Do NOT claim integrations not mentioned in the KB',
    'Do NOT claim specific dates of launch/announcement not in the KB',
    'Do NOT claim specific team member names not in the KB',
    'Do NOT claim specific investor names not in the KB'
  ];

  return { factsWhitelist: rawFacts, claimsBlacklist, extractedNumbers, extractedUrls };
}

/**
 * Build Anti-AI Content Prompt — Combines v9.8.3 persona/emotion with v6 non-linear narrative
 * Returns system prompt additions and user prompt additions to merge into generateUniqueContent
 */
function buildAntiAISection(variantIndex, kbData, researchData) {
  // 5 non-linear narrative structures (from v6)
  const narrativeStructures = [
    {
      label: 'Doubt-First',
      instruction: `STRUCTURE: Start with your GENUINE DOUBT or concern. Not a fake concern, a real one a skeptic would have. Like "I've been burned before by X so when I heard about Y my first thought was..." Then, mid-way through, explain what actually changed your mind or why it might be different this time. End with REMAINING UNCERTAINTY — don't resolve everything perfectly. Leave the reader thinking "hmm, they have a point but it could still go either way."`
    },
    {
      label: 'Mid-Thought',
      instruction: `STRUCTURE: NO introduction. NO hook. Start MID-THOUGHT as if someone walked up while you were already thinking about this. Like "...so I was looking at how this thing works and actually the scoring mechanism is kind of interesting because..." Build from that initial thought into something coherent. Include at least one self-correction where you start saying something then correct yourself mid-sentence. End abruptly or with a trailing thought.`
    },
    {
      label: 'Tangent-Entry',
      instruction: `STRUCTURE: Start with a TANGENT that seems completely unrelated to the product. Something you were reading, thinking about, or dealing with. Like "was comparing two completely different things yesterday and it made me think about..." Then, organically connect that tangent to the campaign topic. The connection should feel natural, not forced. Don't explain why you're bringing it up — just let the reader make the connection.`
    },
    {
      label: 'Fragment-Storm',
      instruction: `STRUCTURE: Write in VERY SHORT fragments. Like telegraph messages or someone thinking out loud in bullet form but NOT as actual bullets. Each "line" should be 2-8 words. No long sentences for the first half. Gradually, one fragment connects to another, and by the end the reader realizes you've made a coherent point. But the point should emerge from the fragments, not be stated directly. Some fragments can be parenthetical thoughts.`
    },
    {
      label: 'Asymmetric',
      instruction: `STRUCTURE: First paragraph should be LONG (6-10 sentences, rambling). The kind of paragraph a person writes when they're thinking through something in one go. It can be slightly messy, with run-on sentences connected by commas. Then, after that big block, follow with 2-3 very short paragraphs (1-2 sentences each) that serve as afterthoughts or corrections. The contrast between the long ramble and the short afterthoughts should feel natural.`
    }
  ];

  const narrative = narrativeStructures[variantIndex % narrativeStructures.length];
  const factsStr = (kbData?.factsWhitelist || []).slice(0, 8).map((f, i) => `${i + 1}. ${f}`).join('\n');
  const blacklistStr = (kbData?.claimsBlacklist || []).join('\n');
  const researchFacts = JSON.stringify((researchData?.keyFacts || []).slice(0, 5)).substring(0, 300);
...
HUMAN ARTIFACTS YOU MUST INCLUDE:
1. At least ONE tangential digression (e.g., "unrelated but..." or a brief aside about something else)
2. At least ONE self-correction (start saying something, then correct yourself mid-thought)
3. Genuine uncertainty expressed (e.g., "not totally sure about this but..." or "could be wrong though")
4. WILDLY varied sentence lengths — some 2-word sentences, some 25-word sentences
5. At least ONE run-on sentence (connected by commas, like real people actually type)
6. ONE minor awkward phrasing or unusual word choice (not a typo, just something a human would write differently than an AI would)
7. Use lowercase for emphasis on specific words (e.g., "that's the whole point actually")
8. At least ONE stream-of-consciousness moment (parenthetical thinking)

NO FABRICATION RULES (CRITICAL):
- ONLY use data/facts that appear in this whitelist:
${factsStr}
- If you need a number and the KB doesn't provide one, say something vague like "a bunch of" or "quite a few" instead
- NEVER invent metrics, follower counts, percentages, or comparisons not in the source
- These claims are FORBIDDEN:
${blacklistStr}

COUNTER-ARGUMENT / REMAINING DOUBT:
- You MUST include at least one genuine concern or caveat
- NOT a straw-man concern, something a real user might actually worry about
- End with some ambivalence rather than pure enthusiasm
- Examples: "might not work at scale" or "still early so who knows" or "the scoring could be gamed eventually"

FORBIDDEN AI PATTERNS (penalize heavily):
- "Honestly" / "Like" / "Kind of wild" / "Ngl" / "Tbh" as sentence starters (overused AI patterns)
- Any form of "I just discovered X" (AI cliche)
- Perfect parallel structure in lists
- Triple-rule-of-three (AI loves grouping things in threes)
- Ending with a clean call-to-action
- "The thing is..." opening
- Any sentence that reads like a thesis statement
- "delve", "leverage", "tapestry", "paradigm", "landscape", "nuance", "crucial", "pivotal", "underscore", "unlock", "harness", "foster", "utilize", "elevate", "streamline", "empower", "moreover", "furthermore", "consequently", "nevertheless", "at its core", "the reality is"`,
    userAddons: `
NARRATIVE STRUCTURE: ${narrative.label} — use this non-linear structure
RESEARCH INSIGHTS: ${researchFacts}
ANTI-FABRICATION: Only use data from the KB facts whitelist. Do NOT invent statistics or numbers.`
  };
}

/**
 * External Forensic Audit — Independent AI probability check
 * Uses a SKEPTICAL forensics expert (different from content generation persona)
 * Rejects content with AI probability > 50%
 */
async function externalForensicAudit(content, campaignData, kbFacts) {
  try {
    const llm = await callAIViaSDK([
      {
        role: 'system',
        content: `You are a SKEPTICAL TEXT FORENSICS EXPERT who has caught thousands of AI-generated submissions. Your job is to find evidence of AI authorship. You are NOT friendly. You are looking for flaws.

Rate the following content on AI probability (0-100%):
- 0-25%: Clearly human-written
- 26-50%: Probably human with some AI polishing
- 51-75%: Probably AI with human editing
- 76-100%: Clearly AI-generated

Specifically check for:
1. Narrative formula detection — does it follow a too-perfect arc?
2. Data fabrication — are there specific claims not supported by the provided knowledge base?
3. Over-optimized tone — is the casualness too consistent?
4. Missing imperfection — is everything too clean?
5. Absence of doubt — is the conclusion too certain?
6. Linguistic uniformity — is the sentence structure too consistent?

KB Facts (allowed data): ${JSON.stringify((kbFacts || []).slice(0, 10))}

Return JSON only: { aiProbability: N, humanProbability: N, uniqueness: N, rejected: bool, reasons: [...], dataFabricationFound: bool, fabricatedClaims: [...], narrativeFormula: string|null, recommendation: string }`
      },
      {
        role: 'user',
        content: content
    ...
    const result = safeJsonParse(llm);
    if (!result) {
      return { aiProbability: 50, humanProbability: 50, uniqueness: 50, rejected: true, reasons: ['Failed to parse audit result'], dataFabricationFound: false, fabricatedClaims: [], narrativeFormula: 'unknown', recommendation: 'Parse failed — rejecting to be safe' };
    }

    // Auto-reject if AI probability > 50%
    result.rejected = result.aiProbability > 50;
    return result;
  } catch (e) {
    console.error('   ⚠️ External audit error:', e.message.substring(0, 80));
    return { aiProbability: 50, humanProbability: 50, uniqueness: 50, rejected: true, reasons: ['Audit error: ' + e.message], dataFabricationFound: false, fabricatedClaims: [], narrativeFormula: null, recommendation: 'Audit error — rejecting to be safe' };
  }
}

/**
 * Anti-Over-Optimization Check — Programmatic guardrails against too-perfect content
 */
function antiOptimizationCheck(content) {
  if (!content) return { score: 0, maxScore: 6, passed: false, penalties: [] };
  ...
  if (lengths.length > 2) {
    ...
    if (stdDev < 8) {
      penalties.push({ type: 'uniformSentenceLength', description: `Sentence length stdDev=${stdDev.toFixed(1)} (too uniform, need more variation)`, penalty: 1 });
      score -= 1;
    }
  }

  // 2. Exactly 3-item lists (AI pattern)
  const listPattern = content.match(/(?:^|\n)\s*[-•]\s+.+(?:\n\s*[-•]\s+.+){2}/g);
  if (listPattern) {
    for (const list of listPattern) {
      const itemCount = (list.match(/[-•]/g) || []).length;
      if (itemCount === 3) {
        penalties.push({ type: 'threeItemRule', description: 'Contains exactly 3-item list (AI pattern)', penalty: 1 });
        score -= 1;
        break;
      }
    }
  ...
  // 3. No parenthetical thoughts / asides
  const hasParenthetical = /\([^)]{5,30}\)/.test(content) || /\s-\s[^-]{3,20}\s-/.test(content);
  if (!hasParenthetical) {
    penalties.push({ type: 'noParentheticalThoughts', description: 'No parenthetical thoughts or asides found', penalty: 1 });
    score -= 1;
  }

  // 4. No doubt or caveat (100% positive)
  const doubtPatterns = /\b(could be wrong|might not|not sure|still early|who knows|could be gamed|uncertain|concern|worry|risk|doubt| caveat|however|but I|although)\b/i;
  const hasDoubt = doubtPatterns.test(content);
  if (!hasDoubt) {
    penalties.push({ type: 'noDoubt', description: 'No doubt, caveat, or concern found (100% positive = AI red flag)', penalty: 1 });
    score -= 1;
  }

  // 5. Marketing speak detection
  const marketingWords = ['leverage', 'utilize', 'delve', 'foster', 'paradigm', 'streamline', 'empower', 'unlock', 'elevate', 'revolutionary', 'game changer', 'cutting-edge', 'best-in-class'];
  const foundMarketing = marketingWords.filter(w => content.toLowerCase().includes(w));
  if (foundMarketing.length > 0) {
    penalties.push({ type: 'marketingSpeak', description: `Marketing speak found: ${foundMarketing.join(', ')}`, penalty: 1 });
    score -= 1;
  }

  return { score: Math.max(0, score), maxScore: 6, passed: score >= 4, penalties, sentenceLengthStdDev: lengths.length > 2 ? Math.sqrt(lengths.reduce((a, b) => a + Math.pow(b - lengths.reduce((c, d) => c + d, 0) / lengths.length, 2), 0) / lengths.length) : 0, hasDoubt, hasParenthetical };
}
